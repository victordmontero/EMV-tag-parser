...
void emvPrint_result(tlvInfo_t* t, int tindex) {
	...
	for (i = 0; i < tindex; i++) {
		...
		{
			printf("%s%s", emvPrint_tabs(tabs + 1), "value: ");
...
void emvPrint_resultString(tlvInfo_t* t, int tindex, char* output) {
	...
	for (i = 0; i < tindex; i++) {
		...
		strcat(output, temp);
		sprintf(temp, "%sTemplate: %X\n", emvPrint_tabs(tabs + 1), t[i].Template);
		strcat(output, temp);
		sprintf(temp, "%sSource: %s\n", emvPrint_tabs(tabs + 1), emvPrint_Source(t[i].Source));
		strcat(output, temp);
		sprintf(temp, "%sType: %s\n", emvPrint_tabs(tabs + 1), t[i].PC ? "Constructed" : "Primitive");
		strcat(output, temp);
		sprintf(temp, "%sRangeLen: %s\n", emvPrint_tabs(tabs + 1), t[i].RangeLen);
		strcat(output, temp);
		...
		if (t[i].PC == CONSTRUCTED) {
			...
			sprintf(temp, "%s%s", emvPrint_tabs(tabs + 1), "Value:");
			strcat(output, temp);
			for (j = 0; j < t[i].tlv.Len; j++) {
				sprintf(temp, " %X", t[i].tlv.Val[j]);
				strcat(output, temp);
			}
			strcat(output, "\n");
			trackLen += sizeof(t[i].tlv.Tag);
